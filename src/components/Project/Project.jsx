...
import { Typography, Card, Image, Pagination, Button } from "antd";
import {
    GithubOutlined,
    FileTextOutlined,
    ToolOutlined,
...
const data = [
    ...
    {
        image: "assets/image/project-gallery-rfdocker.png",
        title: "RfDocker",
        description: "Docker image builder for Robotframework.",
        tools: ["Docker", "Bash", "Robotframework", "Selenium"],
        link: "https://github.com/jh2k2/rfdocker",
    },
];
...
const IndividualCard = (props) => {
    ...
    return (
        ...
        >
            <Meta
                title={state.key}
                description={description_getter(props.data, state.key)}
            />
        </Card>
    );
};
...
const ProjectGallery = (props) => {
    ...
    for (let i = 0; i < Math.ceil(props.data.length / card_per_page); i++) {
        ...
        for (let ii = 0; ii < card_per_page; ii++) {
            if (props.data[ii + i * card_per_page])
                tmp_arr.push(
                    <IndividualCard
                        key={ii + i * card_per_page}
                        data={props.data[ii + i * card_per_page]}
                    />
                );
        }
...
const Project = () => {
    ...
    return (
        <div className="project-content">
            <h1>My Beautiful Project Gallery</h1>
            <Paragraph>
                Everything here can be found on my{"  "}
