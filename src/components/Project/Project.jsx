...
import { Carousel, Divider, Typography, Card, Image, Pagination } from "antd";
...
const tmp_data = [
	...
	},
	{
		...
		link: "https://github.com/jh2k2?tab=repositories",
	},
	{
		image: "https://gw.alipayobjects.com/zos/rmsportal/JiqGstEfoWAOHiTxclqi.png",
...
const IndividualCard = (props) => {
	const [state, setState] = useState({ key: "Project Description" });
	...
	return (
		<Card
			actions={[
				state.key === "Project Description" ? (
					<FileTextFilled style={{ color: "blue" }} key="description" onClick={() => setState({ key: "Project Description" })} />
				) : (
					<FileTextOutlined key="description" onClick={() => setState({ key: "Project Description" })} />
				),
				state.key === "Tools Used" ? (
					<ToolFilled style={{ color: "blue" }} key="tool" onClick={() => setState({ key: "Tools Used" })} />
				) : (
					<ToolOutlined key="tool" onClick={() => setState({ key: "Tools Used" })} />
				),
				<GithubOutlined
					key="github"
					onClick={() => {
			...
			cover={<img alt="Project img" src={props.data.image} />}
			hoverable={true}
		>
	...
	return (
		<div>
			<Carousel autoplay>
				<div>
					<Image
						src="/assets/image/project-carousel-1.jpg"
						...
						onError={(err) => {
							console.log("error occurred when loading jerry's picture");
						}}
					/>
				</div>
				<div>
